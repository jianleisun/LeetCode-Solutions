...
'''
'''
解法一，记下最后一个数字，其他的数字向后移一位，最后把记下的数字放在开头，
如此进行k次。这个解法空间复杂度为O(1)，时间复杂度O(kn)。

解法二，申请一个等大的数组，将移位后的结果存在新申请的数组中。
这个解法空间复杂度为O(n)，时间复杂度为O(n)。+

解法三，右旋k个数字有个等价操作，就是将前k个数字翻转，将剩下的数字也翻转，
最后将整个数组翻转。此时数组与右旋k个数字的结果相同。这个解法空间复杂度为O(1)，时间复杂度为O(n)。
